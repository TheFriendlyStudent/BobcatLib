 ...
 * drivetrain (e.g., lol this reference broke so now you dont get an example). It is intended to be
 * a drop-in replacement for {@link Odometry}; in fact, if you never call {@link
 * AdvancedPoseEstimator#addVisionMeasurement} and only call {@link AdvancedPoseEstimator#update}
 * then this will behave exactly the same as Odometry.
 *
 * <p>{@link AdvancedPoseEstimator#update} should be called every robot loop.
 *
 * <p>{@link AdvancedPoseEstimator#addVisionMeasurement} can be called as infrequently as you want;
...
public class AdvancedPoseEstimator<T> {
  ...
  public void setStateMeasurementStdDevs(Matrix<N3, N1> stateStdDevs) {
    for (int i = 0; i < 3; ++i) {
   ...
   *     AdvancedPoseEstimator#updateWithTime(double,Rotation2d,Object)} then you must use a
