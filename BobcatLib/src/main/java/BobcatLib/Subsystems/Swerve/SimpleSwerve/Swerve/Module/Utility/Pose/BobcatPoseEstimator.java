 ...
 * drivetrain (e.g., { DifferentialDrivePoseEstimator}). It is intended to be a drop-in replacement
 * for { Odometry}; in fact, if you never call { PoseEstimator#addVisionMeasurement} and only call {
 * PoseEstimator#update} then this will behave exactly the same as Odometry.
 *
 * <p>{PoseEstimator#update} should be called every robot loop.
 *
 * <p>{PoseEstimator#addVisionMeasurement} can be called as infrequently as you want; if you never
 * call it then this class will behave exactly like regular encoder odometry.
 *
...
public class BobcatPoseEstimator<T> implements PoseLib {
   ...
   *     {edu.wpi.first.wpilibj.Timer#getFPGATimestamp()}). This means that you should use
