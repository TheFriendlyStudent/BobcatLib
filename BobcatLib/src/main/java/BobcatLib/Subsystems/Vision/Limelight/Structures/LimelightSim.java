...
import BobcatLib.Subsystems.Vision.Components.Utility.apriltag.AprilTag;
import BobcatLib.Subsystems.Vision.Components.Utility.apriltag.AprilTagFieldLayout;
...
public class LimelightSim {
   ...
   *
   * @param limelight {@link LimelightCamera} to use.
   * @param odomPose Supplier of the odom pose.
   * @param fieldLayout {@link AprilTagFieldLayout} to use.
   */
  public LimelightSim(
      LimelightCamera limelight, Supplier<Pose2d> odomPose, AprilTagFieldLayout fieldLayout) {
  ...
  /**
   * Gets the {@link AprilTag} in view of the Limelight, does this by using a Rectangle bounding box
  ...
  public List<AprilTag> getAprilTagsInView() {
    Rectangle2d viewbox =
        new Rectangle2d(m_pose.get(), m_limelightViewDistance, m_limelightViewDistance);
    ...
    llJson.targets_Fiducials =
        (AprilTagFiducial[])
            (tags.stream()
                ...
                .toArray());
    try {
      data.results.setString(
          data.resultsObjectMapper.writer().withDefaultPrettyPrinter().writeValueAsString(llJson));
